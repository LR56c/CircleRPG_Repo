...
namespace THMSV.RPGBuilder.Character
{
    public class RPGBCharacterController : MonoBehaviour
    {
        ...
        [SerializeField] private Transform tpRef, topDownRef, fpRef;
        ...
        [SerializeField] private List<string> camCollisonTags;
        //TODO Change this for your player layer
        public LayerMask playerLayer;
        [SerializeField] private float mouseSensitivity = 1, mouseZoomSpeed, jumpHeight;
        [SerializeField] private int invertMouse = 1;
        [SerializeField] private float minVertAngle, maxVertAngle, minZoom = -0.7f, maxZoom = -6, cameraReturnSpeed = 2;
        ...
        private void Start()
        {
            if (CombatManager.playerCombatInfo != null) InitCharacter();
        }
        ...
        private void InitCharacter()
        {
            ...
            ClickToRotate = true;
            SettingsPanelDisplayManager.Instance.InitControllerButtons();
        }
        ...
        private float GetAxisByName(string name)
        {
            switch (name)
            {
                ...
                {
                    var axis = 0;
                    if (Input.GetKey(CustomInputManager.Instance.controlsKeys[3].currentKey))
                        axis -= 1;
                    if (Input.GetKey(CustomInputManager.Instance.controlsKeys[4].currentKey))
                        axis += 1;
                    return axis;
                }
                case "Vertical":
                {
                    var axis = 0;
                    if (Input.GetKey(CustomInputManager.Instance.controlsKeys[2].currentKey))
                        axis -= 1;
                    if (Input.GetKey(CustomInputManager.Instance.controlsKeys[1].currentKey))
                        axis += 1;
                    return axis;
                }
                default:
        ...
        private void FixedUpdate()
        {
            ...
            if (CAN_MOVE && !isTeleporting && !standTimeActive && !isLeaping &&
                !CombatManager.playerCombatInfo.isStunned() && !CombatManager.playerCombatInfo.isSleeping())
            {
        ...
        public void TeleportToTarget(CombatNode target)
        {
            playerVelocity = Vector3.zero;
            isTeleporting = true;
        ...
        public void CancelDeathAnim()
        {
            anim.SetBool("Dead", false);
        }
        public void TeleportToTarget(Vector3 pos)
        {
            playerVelocity = Vector3.zero;
            isTeleporting = true;
        ...
        private void LateUpdate()
        {
            ...
            {
                if (Input.GetKeyDown(CustomInputManager.Instance.controlsKeys[5].currentKey) && isGrounded)
                {
        ...
        private void ClickController()
        {
            ...
            if (Physics.Raycast(r, out hit, Mathf.Infinity, -1)) navAgent.destination = hit.point;
        }
        #endregion
        ...
        private void Update()
        {
            if(!standTimeActive) speed = thisCombatNode.getCurrentValue("MOVE_SPEED");
            if (navAgent.enabled)
            {
        ...
        public bool standTimeActive, isLeaping;
        public void InitStandTime(float max)
        {
            speed = 0;
            if (currentController == ControllerType.ClickMove)
            {
        ...
        private void ResetStandTime()
        {
            ...
            currentStandTimeDur = 0;
            maxStandTimeDur = 0;
            if (currentController != ControllerType.ClickMove) return;
            navAgent.enabled = true;
            navAgent.isStopped = false;
        }

        public void InitGroundLeap()
        {
            isLeaping = true;
        ...
        public bool isCastingSlowed;
        private float curSpeedPercentage;
        private float speedPercentageTarget;
        private float speedCastSlowRate;
        private float currentCastSlowDur, maxCastSlowDur;

        public void InitCastMoveSlow(float speedPercent, float castSlowDuration, float castSlowRate)
        {
        ...
        private void ResetCastSlow()
        {
            ...
            currentCastSlowDur = 0;
            maxCastSlowDur = 0;
            anim.SetFloat("MoveSpeedModifier", curSpeedPercentage);
        }

        private void HandleCastSlowed()
        {
        ...
        private void HandleCombatStates()
        {
            if (isCastingSlowed && curSpeedPercentage > speedPercentageTarget) HandleCastSlowed();
