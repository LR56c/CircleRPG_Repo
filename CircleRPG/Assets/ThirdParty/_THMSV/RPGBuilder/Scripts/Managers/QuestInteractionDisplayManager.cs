            ...
            if (thisQuestINDEX != -1)
            {
                if (CharacterData.Instance.questsData[thisQuestINDEX].state == QuestManager.questState.abandonned)
                {
                    ...
                    foreach (var t in questToAccept.itemsGiven)
                        InventoryManager.Instance.AddItem(t.itemID,
                            t.count,false, RPGBuilderUtilities.GetItemFromID(t.itemID).randomStats.Count> 0 ?  RPGBuilderUtilities.GenerateRandomItemStats(t.itemID, false, true) : -1);
                }
            }
            else
            {
        ...
        public void CompleteQuest(RPGQuest questToAccept, RPGQuest.QuestRewardDATA rewardDATA)
        {
            ...
            if (thisQuestINDEX != -1)
                if (CharacterData.Instance.questsData[thisQuestINDEX].state == QuestManager.questState.completed)
                {
                    ...
                    {
                        Debug.LogError("Quest items objectives requirements were not met");
                        return;
                    }
                    ...
                    for (var i = 0; i < questREF.rewardsGiven.Count; i++) ProcessQuestReward(questREF.rewardsGiven[i]);
        ...
        private void ProcessQuestReward(RPGQuest.QuestRewardDATA rewardDATA)
        {
            switch (rewardDATA.rewardType)
            {
                case RPGQuest.QuestRewardType.item:
                    InventoryManager.Instance.AddItem(rewardDATA.itemID, rewardDATA.count,false, RPGBuilderUtilities.GetItemFromID(rewardDATA.itemID).randomStats.Count > 0 ? RPGBuilderUtilities.GenerateRandomItemStats(rewardDATA.itemID, false, true) : -1);
                    break;
                ...
                case RPGQuest.QuestRewardType.Experience:
                    LevelingManager.Instance.AddClassXP(rewardDATA.Experience);
                    break;
