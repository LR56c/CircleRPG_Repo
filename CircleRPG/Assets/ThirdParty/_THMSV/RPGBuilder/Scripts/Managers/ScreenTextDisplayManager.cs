    ...
    public class ScreenTextDisplayManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            for (var i = 0; i < CombatTextPoolAmount; i++)
            {
                ...
                //textPanel.SetMobDetails(ClientAPI.GetPlayerObject(), showPlayerName, false, ClientAPI.GetPlayerObject());
        ...
        public void ScreenEventHandler(string eventType, string message, GameObject target)
        {
            ...
            for (var i = 0; i < AllScreenTextDATA.Count; i++)
                if (!AllScreenTextDATA[i].ScreenTextGO.activeInHierarchy)
                {
                    ...
                    AllScreenTextDATA[i].FloatingPanel.ShowCombatText(message, eventType);
                    WeFoundOne = true;
                    break;
                }
                else
            ...
            AllScreenTextDATA[TextToResetID].nodeGO = null;
            AllScreenTextDATA[TextToResetID].RendererReference = null;
            AllScreenTextDATA[TextToResetID].LastTimeUsed = 0;
            ...
            AllScreenTextDATA[TextToResetID].FloatingPanel.ShowCombatText(message, eventType);
        }
