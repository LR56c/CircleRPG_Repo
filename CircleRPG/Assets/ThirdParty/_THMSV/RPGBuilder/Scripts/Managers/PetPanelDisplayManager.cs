...
namespace THMSV.RPGBuilder.Managers
{
    public class PetPanelDisplayManager : MonoBehaviour
    {
        ...
        public Image stayButton, followButton, defendButton, aggroButton;
        ...
        {
            InitButtonsSelection(CombatManager.playerCombatInfo.currentPetsMovementActionType,
                CombatManager.playerCombatInfo.currentPetsCombatActionType);
            UpdateHealthBar();
        ...
        {
            var maxSummons = CombatManager.Instance.getCurrentSummonCount(CombatManager.playerCombatInfo);
            summonCountText.text = "Summons: " + CombatManager.playerCombatInfo.currentPets.Count + " / " + maxSummons;
        }

        public void UpdateHealthBar()
        {
            float totalCurrentPetsHealt = 0, totalmaxPetsHealth = 0;
            foreach (var t in CombatManager.playerCombatInfo.currentPets)
            {
                totalCurrentPetsHealt += t.getCurrentValue("HEALTH");
                totalmaxPetsHealth += t.getCurrentMaxValue("HEALTH");
            }
        ...
        public void selectMovementActionButton(string action)
        {
            ...
            InitButtonsSelection(actionEnum, CombatManager.playerCombatInfo.currentPetsCombatActionType);
            CombatManager.playerCombatInfo.currentPetsMovementActionType = actionEnum;
        }
        ...
        public void selectCombatActionButton(string action)
        {
            ...
            InitButtonsSelection(CombatManager.playerCombatInfo.currentPetsMovementActionType, actionEnum);
            CombatManager.playerCombatInfo.currentPetsCombatActionType = actionEnum;
        }
