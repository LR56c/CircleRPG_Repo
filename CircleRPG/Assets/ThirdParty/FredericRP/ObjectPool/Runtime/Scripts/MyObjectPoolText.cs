...
namespace FredericRP.ObjectPooling
{
  [ExecuteInEditMode]
  [System.Serializable]
  public class MyObjectPoolText : MonoBehaviour
  {
    /// <summary>
    ...
    private static List<MyObjectPoolText> objectPoolList;

    public static MyObjectPoolText GetObjectPool(string id)
    {
    ...
    private void OnEnable()
    {
      if (objectPoolList == null)
        objectPoolList = new List<MyObjectPoolText>();
    ...
    {
      public PoolData(PoolText _obj, float _time)
      {
        gameObjectToPool = _obj;
        time = _time;
      }

      public PoolText gameObjectToPool;
      public float    time;
    }
    ...
    /// </summary>
    private Dictionary<string, List<PoolText>> pooledObjects = new Dictionary<string, List<PoolText>>();
    ...
    public void HeavyAwake()
    {
      ...
      foreach (PoolGameObjectInfo poolObject in poolObjectList)
      {
        ...
        {
          pooledObjects.Add(poolObject.id, new List<PoolText>(poolObject.bufferCount));
          ...
          {
            PoolText newObj = Instantiate(poolObject.prefab, parent);
    ...
    public List<PoolText> GetPooledObjectList(string objectKind)
    {
    ...
    /// Object prefab name
    /// </param>
    public PoolText GetFromPool(string objectName)
    {
    ...
    /// </param>
    public PoolText GetFromPool(string objectName, bool onlyPooled)
    {
      return GetFromPool(objectName, onlyPooled, true);
    }

    PoolText InstantiateObject(PoolGameObjectInfo poolObject)
    {
    ...
    /// If true, it will only return an object if there is one currently pooled.
    /// </param>
    public PoolText GetFromPool(string objectName, bool onlyPooled, bool activate)
    {
      ...
      if (objectPrefabExists)
      {
        ...
        if (pooledObjects.ContainsKey(objectName) && pooledObjects[objectName].Count > 0)
        {
          ...
          {
            PoolText pooledObject = pooledObjects[objectName][0];
    ...
    public void NewPoolKind(string name, PoolText prefab, string tag = "Default", int bufferCount = 1, int maxCount = -1, Transform defaultParent = null)
    {
      ...
      if (poolObjectKind == null)
      {
        ...
        PoolText newObj = Instantiate(poolObjectKind.prefab);
    ...
    /// </returns>
    public bool Pool(PoolText obj, string objectKind = null, Transform poolParent = null)
    {
      ...
      if (poolObjectKind != null)
      {
        ...
        if (!objectPrefabListExists)
          pooledObjects.Add(poolObjectKind.id, new List<PoolText>(1));
      }
    ...
    public void PoolGameObjectList(PoolText[] goList)
    {
    ...
    public void Pool(PoolText obj, float time)
    {
    ...
    void PoolInactiveObjects(string namePrefix, Transform t)
    {
      ...
      {
        Pool(t.gameObject.GetComponent<PoolText>());
      }
    }
#endif
  }
